...
class LabelImageApp():
    ...
    def exit_app(self, save=True) :
        ...
        # Clean up the image logging directory
        current_file = os.path.join(self.log_path,
                                    self.label_df["patient_id"].loc[self.index]+
                                    "_img.npy")

        self.clear_image(current_file)
    ...
    def main_loop(self) :
        ...
        while self.index < nb_patients :
            ...
            # Reset the matplotlib image
            self.clear_image(file_name)
